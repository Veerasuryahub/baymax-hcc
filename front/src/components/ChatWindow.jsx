...
// Quick-start symptom suggestions shown to the user
const QUICK_SUGGESTIONS = [
  "I have a fever and headache",
  "I feel nauseous and have stomach pain",
  "I have skin rash and itching",
  "I have chest pain and shortness of breath",
  "I have cold, cough and sore throat",
  "I feel dizzy and have fatigue",
];

// Shown when bot cannot identify symptoms
const SYMPTOM_HINTS = [
  "fever", "headache", "cough", "skin rash", "fatigue",
  "nausea", "chest pain", "dizziness", "itching", "vomiting",
];
...
const ChatWindow = ({ section, sectionIndex, refresh, onNewChat }) => {
    ...
    return (
      <div className="flex-1 p-4 sm:p-8 bg-[#f8fafc] flex items-center justify-center h-full relative overflow-hidden">
        ...
        <div className="text-center z-10 bg-white/70 backdrop-blur-2xl p-8 sm:p-16 rounded-[32px] sm:rounded-[40px] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.08)] max-w-xl w-full border border-white/40 flex flex-col items-center transition-all hover:scale-[1.02] duration-500">
          ...
          <p className="text-slate-500 mb-6 sm:mb-8 text-base sm:text-xl leading-relaxed font-light px-2">
            Your personal AI healthcare companion. <br className="hidden sm:block" />
            Describe your symptoms and I'll help diagnose.
          </p>

          {/* Quick start suggestions */}
          <div className="w-full mb-6 grid grid-cols-1 sm:grid-cols-2 gap-2">
            {QUICK_SUGGESTIONS.map((s, i) => (
              <button
                key={i}
                onClick={() => { onNewChat && onNewChat(s); }}
                className="text-left text-xs sm:text-sm px-3 sm:px-4 py-2 sm:py-3 rounded-xl bg-slate-50 hover:bg-red-50 border border-slate-200 hover:border-[#E03C31]/30 text-slate-600 hover:text-[#E03C31] transition-all font-medium"
              >
                💬 {s}
              </button>
            ))}
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      let botText;
      if (response.ok) {
        botText = `**🩺 Diagnosis:** ${data.predicted_disease}\n\n**🔍 Symptoms Detected:** ${data.extracted_symptoms.join(", ")}\n\n**💊 Recommendation:** ${data.treatment_recommendation}`;
        setShowHints(false);
      } else {
        const errMsg = data.error || "I encountered an error processing your request.";
        botText = `❌ ${errMsg}`;
        // Show symptom hint chips if bot couldn't identify symptoms
        if (errMsg.includes("couldn't identify")) {
          setShowHints(true);
        }
      }
      ...
      await addMessageToChat(sectionIndex, "bot", "❌ Server Connection Error. The AI Bot may be starting up (free tier). Please wait 30 seconds and try again.");
      setShowHints(true);
    }
  ...
  const handleVoiceInput = () => {
    ...
    recognition.onstart = () => { setIsSpeaking(true); toast.info("🎙️ Listening...", { autoClose: 2000 }); };
    recognition.onresult = (event) => { setMessage(event.results[0][0].transcript); setIsSpeaking(false); };
    recognition.onerror = (event) => {
      setIsSpeaking(false);
      if (event.error === 'not-allowed') toast.error("Microphone access denied.");
      else toast.error("Voice input error. Please try again.");
    };
      ...
      <div className="w-full h-full flex flex-col relative z-10">
        ...
        <div className="bg-white/80 backdrop-blur-md border-b border-slate-200/60 flex items-center justify-between px-4 sm:px-8 py-3 sm:py-5 flex-shrink-0">
          <div className="flex items-center ml-12 md:ml-0">
            <div className="relative mr-3 sm:mr-4">
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-[#E03C31] to-[#ff6b6b] rounded-xl sm:rounded-2xl flex items-center justify-center text-white font-bold text-xl sm:text-2xl shadow-lg ring-2 sm:ring-4 ring-red-50">B</div>
        ...
        <div className="flex-1 overflow-y-auto px-3 sm:px-6 py-4 sm:py-8 scroll-smooth scrollbar-thin">
          <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6">
            ...
            {section.messages.length === 0 && !loading && (
              <div className="flex justify-start">
                <div className="flex max-w-[90%] sm:max-w-[80%] flex-row">
                  <div className="flex-shrink-0 mt-1 mr-2 sm:mr-4">
                    <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-gradient-to-br from-[#E03C31] to-[#ff6b6b] text-white flex items-center justify-center text-xs sm:text-sm font-bold shadow-md">B</div>
                  </div>
                  <div className="flex flex-col items-start">
                    <div className="px-4 sm:px-5 py-3 sm:py-4 rounded-2xl sm:rounded-3xl rounded-tl-none bg-white border border-slate-100/80 shadow-sm text-slate-800 text-[13px] sm:text-[15px] leading-relaxed">
                      <p className="font-semibold mb-2">👋 Hi! I'm Baymax, your AI healthcare companion.</p>
                      <p className="text-slate-500 mb-3">Describe your symptoms and I'll help identify possible conditions. Try one of these:</p>
                      <div className="flex flex-wrap gap-2">
                        {QUICK_SUGGESTIONS.slice(0, 4).map((s, i) => (
                          <button
                            key={i}
                            onClick={() => sendMessage(s)}
                            className="text-xs px-3 py-1.5 rounded-lg bg-red-50 text-[#E03C31] border border-red-100 hover:bg-red-100 transition font-medium"
                          >
                            {s}
                          </button>
                        ))}
            ...
            {section.messages.map((msg, index) => (
              <div key={index} className={`flex w-full group ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`flex max-w-[90%] sm:max-w-[75%] ${msg.sender === "user" ? "flex-row-reverse" : "flex-row"}`}>
                  ...
                  <div className={`relative flex flex-col ${msg.sender === "user" ? "items-end" : "items-start"}`}>
                    <div className={`px-4 sm:px-5 py-3 sm:py-4 rounded-2xl sm:rounded-3xl shadow-sm text-[13px] sm:text-[15px] leading-relaxed transition-all ${
                      msg.sender === "user"
            ...
            {/* Symptom hint chips — shown when bot can't identify symptoms */}
            {showHints && !loading && (
              <div className="flex justify-start pl-12 sm:pl-16">
                <div className="max-w-[85%]">
                  <p className="text-xs text-slate-400 font-semibold mb-2 uppercase tracking-wider">Quick Symptom Options:</p>
                  <div className="flex flex-wrap gap-2">
                    {SYMPTOM_HINTS.map((hint, i) => (
                      <button
                        key={i}
                        onClick={() => sendMessage(`I have ${hint}`)}
                        className="text-xs px-3 py-1.5 rounded-lg bg-white border border-slate-200 text-slate-600 hover:bg-red-50 hover:border-[#E03C31]/30 hover:text-[#E03C31] transition font-medium shadow-sm"
                      >
                        {hint}
                      </button>
                    ))}
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="flex max-w-[75%] items-start animate-fade-in-up">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-gradient-to-br from-[#E03C31] to-[#ff6b6b] text-white flex items-center justify-center text-xs sm:text-sm font-bold mr-2 sm:mr-4 shadow-md flex-shrink-0">B</div>
      ...
      {showReview && (
        <div className="fixed bottom-20 right-4 sm:bottom-24 sm:right-6 w-[calc(100vw-2rem)] sm:w-80 max-w-80 bg-white/90 backdrop-blur-2xl p-5 sm:p-6 rounded-2xl sm:rounded-3xl shadow-[0_20px_40px_rgba(0,0,0,0.15)] border border-white z-50 animate-fade-in-up">
          ...
          <div className="flex space-x-2 sm:space-x-3">
            <button onClick={handleReviewSubmit} className="flex-1 bg-[#E03C31] text-white py-2.5 sm:py-3 rounded-xl font-bold hover:bg-red-600 transition shadow-lg shadow-red-100 text-sm">Send</button>
            <button onClick={() => setShowReview(false)} className="px-3 sm:px-4 py-2.5 sm:py-3 text-slate-500 font-bold hover:bg-slate-100 rounded-xl transition text-sm">Later</button>
          </div>
        </div>
      )}
    </div>
